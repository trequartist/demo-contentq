          ...
          <div className="p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {queryIntentData[selectedQueryType as keyof typeof queryIntentData].queries.map((query, idx) => {
                ...
                const volume = parseInt(query.volume.replace(/[^\d]/g, ''));
                const competitionScore = query.competition === 'Low' ? 3 : query.competition === 'Medium' ? 2 : 1;
                const opportunityScore = volume * competitionScore;
                
                // Determine opportunity level
                const getOpportunityLevel = (score: number) => {
                  if (score >= 50000) return { level: 'High', color: 'red', bg: 'bg-red-50', border: 'border-red-200', text: 'text-red-700' };
                  if (score >= 20000) return { level: 'Medium', color: 'yellow', bg: 'bg-yellow-50', border: 'border-yellow-200', text: 'text-yellow-700' };
                  return { level: 'Low', color: 'green', bg: 'bg-green-50', border: 'border-green-200', text: 'text-green-700' };
                };
                ...
                return (
                  ...
                  >
                    <div className={`border-2 rounded-xl p-5 cursor-pointer transition-all bg-white ${
                      hoveredQuery === query.term 
                        ? `${opportunity.border} shadow-lg scale-105` 
                        : 'border-gray-200 hover:border-gray-300'
                    }`}>
                      {/* Header with Query and Opportunity Badge */}
                      <div className="flex items-start justify-between mb-3">
                        <h4 className="text-sm font-semibold text-gray-900 leading-tight flex-1">{query.term}</h4>
                        <div className={`px-2 py-1 rounded-full text-xs font-bold ${opportunity.bg} ${opportunity.text} ${opportunity.border} ml-2`}>
                          {opportunity.level}
                        </div>
                      </div>
                      ...
                      <div className="space-y-2 mb-3">
                        <div className="flex items-center justify-between">
                          <span className="text-xs text-gray-500">Monthly Volume</span>
                          <span className="text-sm font-semibold text-gray-900">{query.volume}</span>
                        </div>
                        <div className="flex items-center justify-between">
                      ...
                      <div className="mb-3">
                        <div className="flex items-center justify-between text-xs text-gray-500 mb-1">
                          ...
                          <span className="font-medium">{opportunityScore.toLocaleString()}</span>
                        </div>
                        <div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden">
                          <div 
                            className={`h-full rounded-full ${
                              opportunity.color === 'red' ? 'bg-red-500' : 
                              opportunity.color === 'yellow' ? 'bg-yellow-500' : 'bg-green-500'
                            }`}
                            style={{ width: `${Math.min((opportunityScore / 100000) * 100, 100)}%` }}
                          />
                        </div>
                      </div>
                      ...
                      <div className="text-xs text-gray-600 bg-gray-50 rounded p-2">
                        {opportunity.level === 'High' ? 'Priority target - high volume, low competition' :
                         opportunity.level === 'Medium' ? 'Good opportunity - moderate effort required' :
                         'Easy win - low effort, quick results'}
                      </div>
                    </div>
                    ...
                    <AnimatePresence>
                      {hoveredQuery === query.term && (
                        ...
                        >
                          <p>Click to see detailed analysis and content recommendations</p>
                        </motion.div>
                      )}
                    </AnimatePresence>
                  </motion.div>
